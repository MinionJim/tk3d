...
import traceback, threading, multiprocessing, time, os, math, tempfile, shutil, warnings
...
class export (Toplevel, object):
    ...
    def __init__ (self, m):
        ...
        self.b7 = Button (self, text = "Save", font = ("times", 15), command = self.save)
    ...
    def save (self):
        if self.mode.get ():
            if SAVE_VIDEO: self.f = filedialog.asksaveasfile (initialfile = "Figure", defaultextension = "*.mp4", filetypes = (("MP4 video", "*.mp4"), ("AVI video", "*.avi"), ("GIF image", "*.gif")))
            else: self.f = filedialog.asksaveasfile (initialfile = "Figure", defaultextension = "*.gif", filetypes = (("GIF image", "*.gif"),))
        else: self.f = filedialog.asksaveasfile (initialfile = "Figure", defaultextension = "*.png", filetypes = (("PNG image", "*.png"), ("JPEG image", "*.jpg"), ("GIF image", "*.gif"), ("PDF file", "*.pdf")))

        threading.Thread (target = self.thread_save).start ()
    ...
    def save_animation (self):
        ...
        try:
            if self.f:
                ...
                with self.TemporaryDirectory () as image_folder:

                    video_name = self.f.name
                    ...
                    if video_name.split (".") [-1] == "gif":
                        ...
                        video = cv2.VideoWriter (video_name, 0x00000021, 30 / q, (width, height))
    ...
    class TemporaryDirectory:
        # Copied straight from Python 3.6.7's tempfile code
        # (and modified slightly to be compatible with Python 2)
