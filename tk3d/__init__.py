...
if sys.version_info [0] != 3: raise Exception ("An installation of Python 3 is required.")
...
create_cuboid (x1, y1, z1, x2, y2, z2)
    ...
    def create_text (self, *coords, text = ""):
        """
...
For how to use an array, see the class help.
    ...
    def plot_wireframe (self, *coords, step = 1):
        """
...
To delete the object, either call the canvas' delete method or this class' destroy method.
    ...
    def __rotate (self, start, event):
        ...
        if start: self.rorig = (event.x, event.y, *self.angle)
        else:
            ...
            self.rorig = (event.x, event.y, *self.angle)
            self.__redraw ()
