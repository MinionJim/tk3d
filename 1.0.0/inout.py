import tk3d, tkinter.filedialog
...
class save_handeller:
    ...
    def save (self):
        ...
        formed = "\n\n\n".join (formed)
        if not self.file:
            f = tkinter.filedialog.asksaveasfilename (initialfile = "Figure", defaultextension = "*.pypt3", filetypes = (("Python 3D plot file", "*.pypt3"),))
            if f: self.file = f
        if self.file:
    ...
    def open (self):
        f = tkinter.filedialog.askopenfilename (defaultextension = "*.pypt3", filetypes = (("Python 3D plot file", "*.pypt3"),))
        if f:
            self.file = f
